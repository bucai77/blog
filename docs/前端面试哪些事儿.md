...
1. Event Loop 即事件循环，是指浏览器或 Node 的一种解决 JavaScript 单线程运行时不会阻塞的一种机制。
...
- 返回 Promise：Async 函数返回值是 Promise 对象，而 Generator 函数返回的是生成器对象。

## ✅ JavaScript 异步解决方案的发展历程中各自都有哪些优缺点？

为什么需要异步？JavaScript 的单线程存在同步阻塞问题。
...
- React 内部会维护一个是否批量更新的标识。若标识为 ture，则将 setState 加入队列，等待批量更新。若标识为 flase，则直接更新。该标识初始值为 flase，即默认同步更新。
- 为什么异步？若 setState 进入 React 控制流，则标识变为 true，即批量更新形成异步。
- 为什么同步？React 在同步代码的末尾会将标识变为 false，基于 Event Loop 机制，若 setState 进入异步事件，标识无论如何都是 false ，即直接更新形成同步。
...
JSX 是一种类 HTML 动态标签语法，作用是为了能更便捷地使用 JavaScript 构建 UI，其本质是 React.createElement() 的语法糖。
...
虚拟 DOM 是 JavaScript 对象对真实 DOM 的抽象。
...
解决兼容：由于 Taro 蓝牙 Api 有时不能同时满足兼容软件、系统、版本等场景，因此要做封装等兼容处理。
