...
闭包是指能够读取其他函数内部变量的函数，而内存泄露是指访问不到，却依然占居着内存空间的变量。

问题：闭包本身并不会造成内存泄漏，真正的原因是基于低版本 IE 的 GC 中使用的是引用计数算法，造成闭包中出现循环引用时引用双方都不会被回收。

解决：彼时只能通过手动置空来解决，而现代浏览器已经通过清除标记算法取代引用计数算法解决。
...
## ✅ JS 中 0.1 + 0.2 为什么不等于 0.3 以及如何解决？

问题：因为在 0.1 + 0.2 的计算过程中发生了两次精度丢失：第一次是在 0.1 和 0.2 转成双精度二进制浮点数时，第二次在 0.1 和 0.2 转成二进制浮点数后相加时。

解决：Math.js 或 `Math.round(Math.pow(10, m) * number) / Math.pow(10, m)`
...
- 不同的 Mixin 之间存在隐式依赖，不利于维护和理解。
- 不同的 Mixin 之间存在命名空间的冲突。

HOC 相比 Mixin 的优点：

- HOC 拥有天然的组件层级关系。
- HOC 通过属性代理、反向继承等控制组件。
...
- Hooks 解决了 HOC 的嵌套问题，更加简洁。
- Hooks 为函数组件而生，间接解决了类组件的 this 指向、组件复用等问题。

Hooks 的缺点

- 操作不当可能会引用到旧的 state、props 值。
- 重构成本与学习成本较大。
...
- hash 带`#`，history 不带`#`。
- hash 支持低版本浏览器，history 不支持低版本浏览器。
- hash 不依赖服务端，history 需要服务端提供响应网页的支持。
